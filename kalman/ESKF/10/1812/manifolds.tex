...
其中一个特别的元素是幺元，方程\eqRef{equ:axiom_identity}，并因此有一个特殊的切空间是幺元处的切，我们称之为Lie群的Lie代数。 
Lie群结合了光滑流形的局部性质，使我们可以做微积分，利用群的全局性质，实现了远处对象的非线性合成。
...
单位复数之间的增量通过合成和指数映射在切线空间中表示(并且我们将为此定义特殊算子 $\op,\om$)。
...
切向量(红色线段)将流形缠绕在大弧段或测地线(\emph{geodesic})(虚线)上。
...
给定 $\cX(t)$ 在Lie群流形 $\cM$ 上移动的点，它的速度 $\dot\cX=\dparil{\cX}{t}$ 属于 $\cX$ 处与 $\cM$ 切的空间 (\figRef{fig:manifold_tg})，
我们记录为 $\mtanat{\cM}{\cX}$。
...
幺元 $\mtanat{\cM}{\cE}$ 处的切空间称为 $\cM$ 的Lie代数(\emph{Lie algebra})，记录为 $\frak{m}$，
%
\begin{align}
...
%
被找到。
对于乘法群，这将产生新的约束 $\cX\inv\dot\cX + \dot{\cX\inv}\cX = 0$，适用于切于 $\cX$ 的元素(项 $\dot{\cX\inv}$ 是逆项的导数) 。
...
%
Lie代数的元素 $\bftau\hhat$ 具有非平凡结构(斜对称矩阵、虚数、纯四元数，参见 \tabRef{tab:manifolds})，
但对我们来说，关键的方面是，它们可以表示为一些基本元素 $E_i$ 的线性组合，其中 $E_i$ 被称为 $\frak{m}$ 的生成元(\emph{generators}) 
(它们是 $\cX$ 在第 $i$ 个方向上围绕原点的导数)。
在 $\bbR^m$ 中将坐标作为向量进行操作是很方便的，我们简单地记录为 $\bftau$。
...
在本项工作中，我们实施了 $\bbR^m$ 优于 $\frak{m}$ 的偏好，以至于我们定义的大多数操作符和对象(特别是：伴随、Jacobian、扰动及其协方差矩阵，我们很快就会看到)都在 $\bbR^m$ 之上。
...
直观地说， $\exp()$ 将切元素缠绕在大弧段或测地线(\emph{geodesic})跟随的流形上(就像将弦缠绕在球上一样， Figs.~\ref{fig:exponential}， \ref{fig:manifold_z} 和 \ref{fig:manifold_q})。
反向映射是 $\log()$，即展开操作。
...
\if \examples y (对于指数映射在 $\SO(3)$ 和 $S^3$ 中的发展参见 Ex.~\ref{ex:SO3_exp} 和 \ref{ex:S3})。 \else. \fi 
然后将这些求逆以找到对数映射。
%
指数映射的关键性质是 
%
...
我们按照约定\footnotemark~说这个 ${^\cX\!\bftau}$ 是在 $\cX$ 处的局部(\emph{local})坐标系中表示 ---  我们记录到参考坐标系的左上标。
...
\end{align}
%
现在，在方程 \eqRef{equ:lplus} 中 $\Exp({^\cE\!\bftau})$ 在左边，并且我们有 ${{^\cE\!\bftau}\in\mtanat{\cM}{\cE}}$：我们说这个 ${{^\cE}\!\bftau}$ 是在全局(\emph{global})坐标系中表示。
...
我们用方程 \eqssRef{equ:prop_exp,equ:rplus,equ:lplus} 来发展它为 
%
\begin{align*}
\Exp({^\cE}\!\bftau)\cX 
  &= \cX\Exp({^\cX}\!\bftau) \\
...
\end{align}
%
然后发展右手侧结合以标识伴随矩阵 (参见 Ex.~\ref{ex:SE3_adjoint} 和附录)。
伴随矩阵的其它性质是，
%
\begin{align}
...
\end{align}
%
其中 $\bfh\in\bbR^m$, 计算所有列方程 \eqRef{equ:jac_Rn_i} 
...
然而，通过将分子发展成 $\bfh$ 中的线性形式，并将左手侧标识为Jacobian矩阵，该形式可用于计算Jacobian矩阵，即，
%
...
实际上，由于使用右结合(right-)的 $\op$ 和 $\om$ 的操作， $\cX$ 和 $f(\cX)$ 中的变量现在被表示为局部切空间中的向量，即分别在 $\cX\in\cM$ 和 $f(\cX)\in\cN$处的切。
...
$\op$ 对应定义域(domain) $\cM$，并且 $\om$ 对应目标域(codomain)或图像 $\cN$ 。
...
虽然不太可能，但它们有时很有用，因为它们将局部的切映射到全局的切，反之亦然。
...
\caption{围绕着点 $\bar\cX\in\cM$ 的不确定性被正确地表示为该点(红色)处向量空间的切协方差。
...
将积分构造为(小的)离散切步长 $\bftau_k\in\mtanat{\cM}{\cX_{k-1}}$ 的 ``总和(sum)''，即，
%
$
