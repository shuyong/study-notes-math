...
\end{subequations}
%
从方程 \eqRef{equ:ijkQuat} 我们可以看出，在四元数的定义中，我们可以嵌入复数，因此实数和虚数，在实数、虚数和复数确实是四元数的意义上的子集，
%
\begin{align}
...
当将单位四元数解释为方向规范或旋转运算符时，此属性意味着可以使用共轭四元数完成逆旋转。单位四元数总是可以用这种形式写为，
%
\begin{align}
\bfq = \begin{bmatrix}
...
从方程 \eqRef{equ:norm_prod} 开始，赋予乘积运算 $\otimes$ 的单位四元数形成一个非交换群，其中逆与共轭重合。 
...
四元数转换器(\emph{commutator})定义为 $[\bfp,\bfq]\triangleq\bfp\ot\bfq-\bfq\ot\bfp$。我们从方程 \eqRef{equ:quatProdVec} 得到，
%
\begin{align}
...
%我们用实指数的一个推广定义了一般四元数的指数，目的是捕捉其大部分性质。
...
我们在 ~\figRef{fig:rotation3d} 中的旋转，遵循右手规则，说明了一个通用的 3D 矢量 $\bfx$，围绕用单位矢量定义的旋转轴 $\bfu$，旋转 $\phi$ 角度。 
...
%旋转集合作为3参数(3-vectors)矢量上的算子集合有 (a) 保持范数不变，并且 (b) 维持相对方向，也就是说，
%
\begin{align}
...
旋转群通常由旋转矩阵集表示。然而，四元数也是它的一个很好的表示。 
本章的目的是证明这两种表述是同等有效的。
...
\end{align}
%
它们有3自由度(3\,DOF)，并对应于交叉积矩阵，如我们已经在方程 \eqRef{equ:skew} 所介绍的。这就建立了一个一对一映射 $\bfomega\in\bbR^3\leftrightarrow\hatx{\bfomega}\in\so(3)$.
%
...
%-------------------------------------------------------------
\subsubsection{旋转矩阵和旋转矢量： Rodrigues 旋转公式}
%\subsubsection{}
...
\end{align}%
%
这正是矢量旋转公式 \eqRef{equ:vecRotFormula} ，
...
\end{align}
%
这个条件类似于我们在旋转矩阵中遇到的条件，参见方程 \eqRef{equ:Rorthogonal} ，它读为 $\bfR\tr\bfR=\bfI=\bfR\bfR\tr$。我们建议读者暂停一会讨论他们的相似之处。
...
单位四元数的集合在乘法运算下形成一个群。这个群在拓扑上是一个 3-sphere，即 $\bbR^4$ 的单位球的 3D 表面，通常称为 $S^3$ 。
...
这也方便引入角速度矢量 $\bfomega=2\bfOmega\in\bbR^3$，使得方程 \eqRef{equ:qdotOmega} 和 \eqRef{equ:qexpWt} 变成，
%
\begin{align}
...
设 $\bfphi=\phi\bfu$ 是一个旋转矢量表示绕着轴 $\bfu$ 的 $\phi$\,rad 的旋转，
然后，
...
\end{subequations}
%
对于小角度四元数，方程 \eqRef{equ:qvec} 发散。我们然后对 $\arctan()$ 函数使用泰勒级数并截断，得到，
%
\begin{equation}
\Log(\bfq) = \theta\bfu 
...
我们终于在这里证明用四元数旋转矢量的假设方程 ~\eqRef{equ:qrot} ，  
...
%
%
为了证明这个双积确实执行了所需的向量旋转，我们使用方程 \eqRef{equ:quatProdVec} ，
...
\paragraph{rotations 在 $\bbR^4$ 中的等倾旋转：} 
%
...
\subsubsection{在 $SO(3)$ 中的右 Jacobians 矩阵}
...
在极限中，$\delta\bfphi$ 作为 $\dth$ 的函数的变化定义了 Jacobians 矩阵
%
\begin{align}
...
右 Jacobians 矩阵及其逆矩阵可以进行闭式计算 \citep[page 40]{CHIRIKJIAN-12}，
%
\begin{align}
...
这些表达式当然与方程 \eqRef{equ:qdot} 和 \eqRef{equ:Rdot}相同，是在旋转群 $SO(3)$ 的框架下扩展起来的。 
...
四元数形式的累积旋转是通过积分适合于旋转速率定义的微分方程来完成的，即，方程 \eqRef{equ:qdotLocal} 用于局部旋转速率定义，和方程 \eqRef{equ:qdotGlobal} 用于全局旋转速率定义。 
...
\end{align}
%
在这个表达式中，和数的第一项是中间的零阶积分器 \eqRef{equ:intFirstC}。
